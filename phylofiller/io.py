...
def read_metadata(fp_metadata: str, fp_assemblyprefix: str=None):
    ...
    if len(conflicting_col_names) > 0:
        raise ValueError('Conflicting column name(s) in your metadata: %s please rename!' % ', '.join(map(lambda x: '"%s"' % x, conflicting_col_names)))

    if meta.index.name != EXPECTED_COLUMNS[0]:
        raise ValueError('Header of first column must be "%s"' % EXPECTED_COLUMNS[0])
    ...
    ambig_organisms = meta.loc[meta.index.value_counts() > 1]['__line_number'].reset_index().groupby('organism').apply(lambda x: x['__line_number'].values)
    if ambig_organisms.shape[0] > 0:
        raise ValueError('%i organisms have been re-defined in your metadata. Please fix!\n%s' % (ambig_organisms.shape[0], str(ambig_organisms)))

    # add assembly prefix if given
    if fp_assemblyprefix is not None:
        meta['fp_assembly'] = meta['fp_assembly'].apply(lambda x: os.path.abspath('%s%s' % (fp_assemblyprefix, x)))
    # check if given assembly file paths exists
    meta['__file_exists'] = meta['fp_assembly'].apply(lambda x: os.path.exists(x))
    if meta[~meta['__file_exists']].shape[0] > 0:
        raise ValueError("Cannot find files for following assemblies:\n%s" % str(meta[~meta['__file_exists']]))
